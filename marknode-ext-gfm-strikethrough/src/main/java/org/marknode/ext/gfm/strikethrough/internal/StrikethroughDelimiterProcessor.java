...
public class StrikethroughDelimiterProcessor implements DelimiterProcessor {
  ...
  public int getDelimiterUse(int openerCount, int closerCount) {
    if (openerCount >= 2 && closerCount >= 2) {
      ...
      // Can happen if a run had 3 delimiters before, and we removed 2 of them in an earlier
      // processing step.  So just use 1 of them, see corresponding handling in process method.
      return 1;
    }
  }
  ...
  public void process(Text opener, Text closer, int delimiterCount) {
    // Can happen if a run had 3 or more delimiters, so 1 is left over. Don't turn that into
    // strikethrough, but preserve original character.
    if (delimiterCount == 1) {
