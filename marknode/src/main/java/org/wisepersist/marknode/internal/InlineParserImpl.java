...
import org.wisepersist.marknode.internal.inline.AsteriskDelimiterProcessor;
import org.wisepersist.marknode.internal.inline.UnderscoreDelimiterProcessor;
import org.wisepersist.marknode.internal.util.Escaping;
import org.wisepersist.marknode.internal.util.Html5Entities;
import org.wisepersist.marknode.internal.util.Parsing;
import org.wisepersist.marknode.node.Code;
import org.wisepersist.marknode.node.HardLineBreak;
import org.wisepersist.marknode.node.HtmlInline;
import org.wisepersist.marknode.node.Image;
import org.wisepersist.marknode.node.Link;
import org.wisepersist.marknode.node.Node;
import org.wisepersist.marknode.node.SoftLineBreak;
...
public class InlineParserImpl implements InlineParser {
  ...
  private static final String
      HTMLTAG = "(?:" + Parsing.OPENTAG + "|" + Parsing.CLOSETAG + "|" + HTMLCOMMENT
  ...
  public InlineParserImpl(Set<Character> specialCharacters,
  ...
  public static Set<Character> calculateDelimiterCharacters(Set<Character> characters) {
    Set<Character> hashSet = Sets.newHashSet();
  ...
  public static Set<Character> calculateSpecialCharacters(Set<Character> delimiterCharacters) {
    Set<Character> hashSet = Sets.newHashSet();
    hashSet.addAll(delimiterCharacters);
    hashSet.add('\n');
    hashSet.add('`');
    hashSet.add('[');
    hashSet.add(']');
  ...
  public static Map<Character, DelimiterProcessor> calculateDelimiterProcessors(
    ...
    addDelimiterProcessors(
        Arrays.<DelimiterProcessor>asList(new AsteriskDelimiterProcessor(),
                                          new UnderscoreDelimiterProcessor()), map);
  ...
  private boolean parseInline() {
    ...
    switch (c) {
      ...
      case '&':
        ...
        break;
      default:
        boolean isDelimiter = delimiterCharacters.contains(c);
        if (isDelimiter) {
  ...
  private boolean parseNewline() {
    ...
    if (lastChild != null && lastChild instanceof Text && ((Text) lastChild).getLiteral()
        .endsWith(" ")) {
  ...
  private boolean parseString() {
    ...
    while (index != length) {
      if (specialCharacters.contains(input.charAt(index))) {
        break;
      }
      index++;
    }
